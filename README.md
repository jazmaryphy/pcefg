...
where $q_{k}$ and $\mathbf{x}_k = \mathbf{r}_0 - \mathbf{r}_k = ({x_{1k}}, {x_{2k}}, {x_{2k}})$ are the charge and position vector of the $k$-th ion located at distance $r_k = \lvert \mathbf{x}_k \rvert$ from the origin at the site of interest ($\mathbf{r}_0$).
